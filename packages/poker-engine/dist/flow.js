...
function settleAndEnd(state) {
    ...
    if (contenders.length === 1) {
        ...
        for (const pot of pots) {
            ...
            let best = null;
            let winners = [];
            const classOrder = { high_card: 0, pair: 1, two_pair: 2, three_kind: 3, straight: 4, flush: 5, full_house: 6, four_kind: 7, straight_flush: 8 };
            ...
            if (remainder > 0)
                distributeRemainderToWinners(winners, remainder, (s.buttonIndex + 1) % s.seats.length, s.seats.length, (idx) => { s.seats[idx].stack += 1; });
        }
        s.pot.main = 0;
    }
    s.status = 'hand_over';
