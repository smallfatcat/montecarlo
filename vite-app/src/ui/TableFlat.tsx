...
export function TableFlat() {
  ...
  return (
    <div id="table-flat" style={{ position: 'relative', minHeight: '75vh', paddingTop: 5, paddingBottom: 10 }}>
      {/* Controls (same as multi view) */}
      <div className="controls" id="controls" ref={controlsRef} style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 0 }}>
        ...
        <button id="simulate-button" onClick={() => {
          ...
          worker.postMessage({ type:'run', options: { numHands: handsPerRun, numPlayers, deckCount: decks, reshuffleCutoffRatio: 0.2, initialBankrolls: bankrolls, casinoInitial: 10000, betsBySeat: Array.from({ length: numPlayers }, () => bet), } })
        }}>Simulate</button>
      ...
      <LayoutGroup key={roundId}>
        <div id="dealer-lane" style={{ display: 'flex', gap: 8, justifyContent: 'center', marginTop: 8 }}>
        ...
        {revealed ? (
          <div id="dealer-total" className={`hand-total ${evaluateHand(table.dealerHand).isBust ? 'is-bust' : evaluateHand(table.dealerHand).isBlackjack ? 'is-bj' : evaluateHand(table.dealerHand).isSoft ? 'is-soft' : ''}`} style={{ textAlign: 'center', marginTop: 4 }}>
            {handTotalLabel(evaluateHand(table.dealerHand))}
          </div>
        ) : null}

        <div id="players-lane" style={{ position: 'absolute', bottom: 150, left: 16, right: 16, display: 'flex', gap: 16, justifyContent: 'center', alignItems: 'flex-end', overflowX: 'auto', paddingBottom: 4 }}>
          {seats.map((seat, si) => (
            <div key={`seat-${si}`} id={`seat-${si}`} className={`seat ${table.status === 'seat_turn' && activeSeat === si ? 'active' : ''}`} style={{ padding: 8, textAlign: 'center', minWidth: 240 }}>
              <div style={{ fontWeight: 700, opacity: 0.85, marginBottom: 4 }}>{si === 0 ? 'Player' : `CPU ${si}`}</div>
              {seat.hands.map((hand, hi) => {
                ...
                return (
                  <div key={`seat-${si}-hand-${hi}`} id={`seat-${si}-hand-${hi}`} className="hand-row" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, justifyContent: 'center', marginBottom: 6 }}>
      ...
      <div id="flat-actions" style={{ position: 'absolute', left: '50%', bottom: 12, transform: 'translateX(-50%)', display: 'flex', gap: 8, alignItems: 'center', background: 'rgba(0,0,0,0.15)', padding: 8, borderRadius: 10 }}>
        ...
        <label style={{ marginLeft: 8 }}><input type="checkbox" checked={autoPlay} onChange={(e) => setAutoPlay(e.target.checked)} /> Auto</label>
      </div>
    </div>
  )
}
