  ...
  const appendEvent = (handId: number, e: HistoryEvent) => {
    ...
    const line = (() => {
      switch (e.type) {
        ...
        case 'hand_setup': return `Setup: BTN ${e.buttonIndex} stacks [${e.seats.map((s: any) => s.stack).join(', ')}] deck ${e.deckRemaining}/${e.deckTotal}`
        ...
        case 'results': {
          const parts = e.perSeat
            .filter((x: any) => x.delta !== 0)
            .map((x: any) => `Seat ${x.seat} ${x.delta > 0 ? '+' : ''}${x.delta}${x.revealed ? ` (${x.revealed})` : ''}`)
          return parts.length ? `Result: ${parts.join(' • ')}` : `Result: no net changes`
        }
        case 'hand_end': return `Hand #${e.handId} end`
        default: return `Unknown event: ${(e as any).type}`
      }
    })()
