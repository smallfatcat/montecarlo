...
export function PokerSeat(props: PokerSeatProps) {
  ...
  return (
    <div id={`${idPrefix}-${seatIndex}`} style={{ ...baseStyle, ...containerStyle }}>
      ...
      <div id={`${idPrefix}-stack-${seatIndex}`} style={{ textAlign: 'center', fontSize: 12, opacity: 0.9 }}>Stack: {seat.stack}</div>
      <div id={`${idPrefix}-bets-${seatIndex}`} style={{ textAlign: 'center', fontSize: 12, opacity: 0.9 }}>Bet: {seat.committedThisStreet} • In pot: {seat.totalCommitted}</div>
      {showPerSeatEquity && (
