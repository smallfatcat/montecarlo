...
export function PokerStackSlider({
  ...
  const computePctFromBet = (bet: number) => {
    const high = Math.max(0, Math.floor(stack))
    if (high === 0) return 0
  ...
  // When legal bounds change, rebase the slider to current bet
  useEffect(() => {
  ...
  return (
    <>
      <div id="control-stackSlider" style={{ position: 'absolute', left: layout.left, top: layout.top, transform: 'translate(-50%, -50%)', ...sized(layout) }} {...makeDragHandlers('stackSlider', layout)}>
      ...
      {(() => {
        ...
        return (
          <div id="control-stackSliderLabel" style={{ position: 'absolute', left: computedLabelLayout.left, top: computedLabelLayout.top, transform: 'translate(-50%, -50%)', pointerEvents: editLayout ? 'auto' : 'none', ...sized(computedLabelLayout) }} {...makeDragHandlers('stackSliderLabel', computedLabelLayout)}>
            <span style={{ fontSize: 12, opacity: 0.9, userSelect: 'none' }}>Stack% {(() => {
              const high = Math.max(0, Math.floor(stack))
              if (high === 0) return '(0%)'
