...
/**
 * Check if two cards are suited (same suit)
 */
...
/**
 * Get the highest rank on the board
 */
...
export function hasOpenEndedStraightDraw(hole: Card[], community: Card[]): boolean {
  ...
  if (vals.includes(rankVal("A")) && vals.includes(rankVal("2")) && 
      vals.includes(rankVal("3")) && vals.includes(rankVal("4"))) return true;
  
  return false;
}

/**
 * Analyze the strength of a made hand
 */
export function analyzeMadeHand(hole: Card[], community: Card[]): {
  ...
  return { 
    pair, topPair, overPair, twoPair, trips, quads, fullHouse, 
