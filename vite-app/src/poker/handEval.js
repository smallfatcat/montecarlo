...
function isSequential(sortedValsDesc) {
    for (let i = 1; i < sortedValsDesc.length; i += 1) {
        if (sortedValsDesc[i - 1] - 1 !== sortedValsDesc[i])
            return false;
    }
    return true;
}
...
export function compareEvaluated(a, b) {
    const order = [
        "high_card",
        "pair",
        "two_pair",
...
export function formatEvaluated(ev) {
    switch (ev.class) {
        case 'straight_flush':
...
export function pickBestFive(allCards) {
    if (allCards.length !== 7)
        throw new Error('pickBestFive expects 7 cards');
    let best = null;
    let bestIdx = [];
    for (let a = 0; a <= 2; a += 1) {
