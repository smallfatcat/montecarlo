...
function __sumStacks(state) {
    return state.seats.reduce((sum, s) => sum + Math.max(0, Math.floor(s.stack)), 0);
}
function __getBaseline(state) {
    return state.__baselineChips;
}
function __setBaseline(state) {
    ;
    state.__baselineChips = __sumStacks(state) + Math.max(0, Math.floor(state.pot.main));
}
function __assertConservation(state, context, expectedRake = 0) {
...
export function createInitialPokerTable(numSeats, cpuSeats, startingStack = CONFIG.poker.startingStack, shoe) {
...
function postBlind(state, seatIndex, amount) {
...
function commitChips(state, seat, amount) {
    ...
    try {
        __assertConservation(state, 'commitChips', 0);
    }
    catch (e) {
        throw e;
    }
...
function settleAndEnd(state) {
    ...
    if (contenders.length === 1) {
        ...
        s.pot.main = 0;
        try {
            __assertConservation(s, 'settleAndEnd (single contender)', rake);
        }
        catch (e) {
            throw e;
        }
        ...
        for (const pot of pots) {
            if (pot.amount <= 0 || pot.eligibleSeatIdxs.length === 0)
                continue;
        ...
        s.pot.main = 0;
        try {
            __assertConservation(s, 'settleAndEnd (multi)', totalRake);
        }
        catch (e) {
            throw e;
        }
...
export function computePots(state) {
    ...
    const allInLevels = Array.from(new Set(totals
        .filter((x) => seats[x.idx].isAllIn || seats[x.idx].stack === 0)
        .map((x) => x.total))).sort((a, b) => a - b);
    ...
    if (Number.isFinite(minTotal) && minTotal > 0)
        levels.push(minTotal);
    for (const v of allInLevels) {
        if (levels[levels.length - 1] !== v)
            levels.push(v);
    }
    if (levels.length === 0 || levels[levels.length - 1] < maxTotal)
        levels.push(maxTotal);
...
function __computePotsDistinct(state) {
    const seats = state.seats;
    const totals = seats.map((s, i) => ({ idx: i, total: Math.max(0, Math.floor(s.totalCommitted)) }))
        .filter((x) => x.total > 0);
    if (totals.length === 0)
        return [];
    const levels = Array.from(new Set(totals.map((x) => x.total))).sort((a, b) => a - b);
    const pots = [];
    let prev = 0;
    let carry = 0;
    for (const level of levels) {
...
function prevActiveSeatIndex(seats, startExclusive) {
...
export function __test__advanceStreet(state) {
    return advanceStreet(state);
}
export function __test__finalizeShowdown(state) {
    return settleAndEnd(state);
}
function distributeRemainderToWinners(winners, remainder, startFrom, seatsLen, add) {
    let idx = startFrom;
    while (remainder > 0) {
...
function computeRake(amount) {
    if (!CONFIG.poker.showRakeInUI)
        return 0;
